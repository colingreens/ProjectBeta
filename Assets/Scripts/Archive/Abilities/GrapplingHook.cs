...
namespace MetroidVaniaTools
{
    public class GrapplingHook : Abilities
    {
        ...
        {
            if (input.FireOneHeld() && weapon.currentProjectile != null && 
                weapon.currentProjectile.GetComponent<Projectile>().fired && 
                weapon.currentProjectile.tag == "GrapplingHook")
            {
        ...
        {
            if (!input.FireOneHeld() || weapon.currentTimeTillChangeArms <= 0 || removed)
            {
                removed = true;
                if (connected)
                {
